...
Build the Erfurt datasets for the Hide-and-Seek app from open data:
  ...
  * Stops  -> app/src/data/stops.erfurt.ts
      All tram stops (lines 1-6) + the stops of bus lines 9, 10, 30, 51, 80, 90,
      taken from the VMT GTFS feed (EVAG, agency_id 119), grouped by name.
  ...
  * Districts -> app/public/erfurt/stadtteile.geojson
      The 53 Erfurt Ortsteile from the Geoportal (already WGS84 GeoJSON).
...
def main() -> None:
    ...
    print(f"Hideouts (grouped stops): {len(hideouts)}")
    ...
    ts = (
        ...
        "// All tram stops (lines 1-6) + bus lines 9, 10, 30, 51, 80, 90.\n"
    ...
    # 6. Line geometry: longest trip per line -> ordered, de-duplicated coords.
    features = []
    ...
    for line in lines:
        ...
        if best_trip is None:
            continue
        coords = []
        for _seq, sid in sorted(trip_seq[best_trip], key=lambda x: x[0]):
            info = stop_info.get(sid)
            if not info:
                continue
    ...
    # 7. Stadtteile passthrough (already WGS84 GeoJSON, props STADTTE_ID/NAME).
    ...
    print(f"Wrote {OUT_STADTTEILE} ({len(stadtteile.get('features', []))} Ortsteile)")
